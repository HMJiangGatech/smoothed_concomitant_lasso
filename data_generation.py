...
def generate_data_cmr(n_samples, n_features, nresponse, sigma=1., snr=2., sparsity=0.8,
                  corr=0.5, random_state=42):
    ...
    covar = toeplitz(vect, vect)
    X = rng.multivariate_normal(np.zeros(n_features), covar, n_samples)
    beta = rng.laplace(size=n_features * nresponse)
    n_zeros = int(n_features * nresponse * sparsity)
    mask = rng.choice(range(n_features * nresponse), n_zeros, replace=False)
    beta[mask] = np.zeros(n_zeros)
    beta = beta.reshape([n_features,nresponse])

    # scale to precribed snr
    # scale = np.sqrt(np.dot(beta.T, np.dot(covar, beta)) / (snr * sigma ** 2))
    scale = np.sqrt(sigma ** 2 * snr / np.dot(beta.T, np.dot(covar, beta)).trace())
    beta *= scale

    y = np.dot(X, beta) + sigma * rng.normal(0, 1, n_samples*nresponse).reshape([n_samples,nresponse])
