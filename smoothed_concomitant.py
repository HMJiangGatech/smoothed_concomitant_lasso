...
def SC_path_CMR(X, y, lambdas, sigma_0=None, eps=1e-4, max_iter=5000, f=10):
    ...
    n_samples, n_features = X.shape
    n_samples, n_ respense = y.shape
    sqrt_n = np.sqrt(n_samples)
    betas = np.zeros((n_lambdas, n_features, n_response))
    beta_init = np.zeros([n_features, n_response], order='F')
    residual = y.copy()
    sigmas = np.zeros(n_lambdas)
    gaps = np.ones(n_lambdas)
    n_iters = np.zeros(n_lambdas)
